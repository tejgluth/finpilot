...
def evaluate_risk(
    ...
    if not signals:
        return RiskCheckResult(False, 0.0, "No agent signals available.")

    directional_signals = [signal for signal in signals if signal.action in {"BUY", "SELL"}]
    if not directional_signals:
        return RiskCheckResult(False, 0.0, "No actionable agent signals available.")
    ...
    if avg_confidence < agent_settings.min_confidence_threshold:
        return RiskCheckResult(False, 0.0, "Aggregate confidence below the configured minimum threshold.")
    ...
    if total_directional_strength <= 0:
        return RiskCheckResult(False, 0.0, "Directional conviction was too weak to size a position.")
    ...
    return RiskCheckResult(True, proposed_pct, "Risk checks passed.")
